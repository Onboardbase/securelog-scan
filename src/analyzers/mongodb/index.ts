...
const analyzeMongoDBConnection = async (
  ...
  try {
    ...
    console.error(chalk.red("[x] Error: "), (error as Error).message);
    process.exit(1);
  }
};

// Extract the database name from the connection string
const extractDatabaseName = (connectionString: string): string | null => {
...
const getCurrentUser = async (client: MongoClient): Promise<void> => {
  ...
  try {
    ...
    console.error(
      chalk.red("[x] Error fetching current user:"),
...
const displayDatabaseInfo = async (db: Db): Promise<void> => {
...
const displayCollectionsInfo = async (db: Db): Promise<void> => {
...
const displayIndexesInfo = async (db: Db): Promise<void> => {
  const collections = await db.listCollections().toArray();
...
const displayUserRolesAndPrivileges = async (
  client: MongoClient
): Promise<void> => {
  const adminDb = client.db().admin();
  ...
  try {
    ...
    const userRoles = users.users.map((user: Record<string, any>) => ({
      user: user.user,
      roles: user.roles
        .map((role: Record<string, any>) => `${role.role}@${role.db}`)
        .join(", "),
    }));
    ...
    userRoles.forEach((userRole: Record<string, any>) => {
...
const mongodbAnalyzer = (mongoConnectionString: string): void => {
