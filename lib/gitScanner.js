...
const fs = require("fs");
const { getActualGitURLFilePath, maskString } = require("./util");
      ...
      if (scanResponse) {
        if (Array.isArray(scanResponse) && scanResponse.length) {
          scanResponse.map((response) =>
            logPotentialSecret(response, commitInfo, filePath, isUrl, mask)
          );
        } else
...
const scanGitCommitsForSecrets = async ({
  ...
  if (changed) {
    ...
    scanChangedLinesInFiles(changedFiles, startDirectory, verify, core, mask);
    return;
  }
