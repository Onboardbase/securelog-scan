...
const scan = async (verify, data) => {
  const azureClientSecretMatches = data.matchAll(clientSecretPat, -1);
  const azureTenantIdMatches = data.matchAll(tenantIDPat, -1);
  const azureClientIdMatches = data.matchAll(clientIDPat, -1);
  const result = { detectorType: "Azure", verified: false };

  for (const azureClientSecretMatch of azureClientSecretMatches) {
    if (azureClientSecretMatch.length !== 3) continue;
    const azureSecretAccessKey = azureClientSecretMatch[2].trim();
    ...
    for (const tenantId of azureTenantIdMatches) {
      ...
      const azureTenantId = tenantId[2].trim();
      ...
      for (const clientId of azureClientIdMatches) {
        ...
        const azureClientId = clientId[2].trim();
