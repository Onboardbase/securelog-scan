...
const { detectSecretsInLine } = require("./util");
      ...
      if (possibleSecret) {
        for (const [company, regex] of Object.entries(regexPatterns)) {
          ...
          if (regex.test(possibleSecret)) {
             ...
             */
            let matchedValue = "";
            const stringHasWhiteSpaces = possibleSecret.split(" ").length > 1;
            if (stringHasWhiteSpaces) {
              matchedValue = possibleSecret
                .split(" ")
                .find((singleSecret) => regex.test(singleSecret.trim()));
            }
            ...
            console.log(
              `${chalk.bold("Raw Value:")} ${matchedValue || possibleSecret}\n`
            );
            break;
          }
        }
  ...
  for (const file of files) {
    ...
    if (file.isDirectory()) {
      ...
      if (
        ...
        fileExtension === ".txt" ||
        fileExtension.match(
          /\.(js|ts|py|rb|go|php|java|cs|cpp|c|sh|bat|yaml|yml|rs|ini|cfg)$/
        )
      ) {
